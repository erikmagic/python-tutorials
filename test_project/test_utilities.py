...
class TestUtilities:
    ...
    def test_translate_to_jadensmith_single_word(self):
        ...
        actual_value = self.converter.translate_to_jadensmith(sentence)
    ...
    def test_translate_to_internet_meme_raises_value_error(self):
        with pytest.raises(ValueError):
            self.converter.translate_to_internet_meme(999)

    def test_translate_to_internet_meme_single_word(self):
        sentence = "bonjour"
        expected_value = "BoNjOuR"

        actual_value = self.converter.translate_to_internet_meme(sentence)
